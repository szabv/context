...
Description: Learn how to use realtime to stream data from Inngest functions to your users.';
...
TypeScript SDK v3.32.0+" /> <VersionBadge version="Go SDK v0.9.0+" /> <VersionBadge version="Python v0.5.9+" />
...
inngest.createFunction(
  ...
  async ({ event, step, publish }) => {
    ...
    await step.run('generate-response', () => {
      llm.generateResponse(event.data.prompt);
      await publish({
