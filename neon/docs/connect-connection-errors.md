...
- [The endpoint ID is not specified](connect-connection-errors.md#the-endpoint-id-is-not-specified)
- [Password authentication failed for user](connect-connection-errors.md#password-authentication-failed-for-user)
- [Couldn't connect to compute node](connect-connection-errors.md#couldnt-connect-to-compute-node)
- [Can't reach database server](connect-connection-errors.md#cant-reach-database-server)
- [Error undefined: Database error](connect-connection-errors.md#error-undefined-database-error)
- [Terminating connection due to administrator command](connect-connection-errors.md#terminating-connection-due-to-administrator-command)
- [Unsupported startup parameter](connect-connection-errors.md#unsupported-startup-parameter)
- [You have exceeded the limit of concurrently active endpoints](connect-connection-errors.md#you-have-exceeded-the-limit-of-concurrently-active-endpoints)
- [Remaining connection slots are reserved for roles with the SUPERUSER attribute](connect-connection-errors.md#remaining-connection-slots-are-reserved-for-roles-with-the-superuser-attribute)
- [Relation not found](connect-connection-errors.md#relation-not-found)
- [Postgrex: DBConnection ConnectionError ssl send: closed](connect-connection-errors.md#postgrex-dbconnection-connectionerror-ssl-send-closed)
- [query_wait_timeout SSL connection has been closed unexpectedly](connect-connection-errors.md#querywaittimeout-ssl-connection-has-been-closed-unexpectedly)
- [The request could not be authorized due to an internal error](connect-connection-errors.md#the-request-could-not-be-authorized-due-to-an-internal-error)
- [Terminating connection due to idle-in-transaction timeout](connect-connection-errors.md#terminating-connection-due-to-idle-in-transaction-timeout)
- [DNS resolution issues](connect-connection-errors.md#dns-resolution-issues)
...
- Try [connecting with psql](connect-query-with-psql-editor.md) to see if a connection can be established.
- Review the strategies in [Connection latency and timeouts](connect-connection-latency.md) for avoiding connection issues due to compute startup time.

If the connection issue persists, please reach out to [Support](introduction-support.md).
...
When you connect to an idle compute, Neon automatically activates it. Activation typically happens within a few seconds. If the error above is reported, it most likely means that the Prisma query engine timed out before your Neon compute was activated. For dealing with this connection timeout scenario, refer to the [connection timeout](guides-prisma.md#connection-timeouts) instructions in our Prisma documentation. Our [connection latency and timeout](connect-connection-latency.md) documentation may also be useful in addressing this issue.
...
Neon has a default limit of 20 concurrently active computes to prevent resource exhaustion. The compute associated with the default branch is exempt from the [concurrently active compute limit](reference-glossary.md#concurrently-active-compute-limit), ensuring that it is always available. When you exceed the limit, additional computes beyond the limit will remain suspended and you will see this error when attempting to connect to them. You can suspend other active computes and try again. Alternatively, if you encounter this error often, you can reach out to [Support](introduction-support.md) to request a `max_active_endpoints` limit increase.
...
- Use a larger compute, with a higher `max_connections` configuration. See [How to size your compute](manage-computes.md#how-to-size-your-compute).
- Enable [connection pooling](connect-connection-pooling.md).

If you are already using connection pooling, you may need to reach out to Neon Support to request a higher `default_pool_size` setting for PgBouncer. See [Neon PgBouncer configuration settings for more information](connect-connection-pooling.md#neon-pgbouncer-configuration-settings).

## Relation not found

This error is often encountered when attempting to set the Postgres `search_path` session variable using a `SET search_path` statement over a pooled connection. For more information and workarounds, please see [Connection pooling in transaction mode](connect-connection-pooling.md#connection-pooling-in-transaction-mode).
...
To check for a DNS resolution issue, you can run `nslookup` on your Neon hostname, which is the part of your Neon database [connection string](reference-glossary.md#connection-string) starting with your endpoint ID (e.g., `ep-cool-darkness-a1b2c3d4`) and ending with `neon.tech`. For example:
