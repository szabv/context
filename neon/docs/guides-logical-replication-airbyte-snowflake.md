...
> The document outlines the process for Neon users to replicate data from Neon to Snowflake using Airbyte, detailing the setup and configuration steps necessary for establishing a data pipeline between these platforms.

## Source

- [Replicate data to Snowflake with Airbyte HTML](https://neon.com/docs/guides/logical-replication-airbyte-snowflake): The original HTML version of this documentation
...
- A source [Neon project](manage-projects.md#create-a-project) with a database containing the data you want to replicate. If you're just testing this out and need some data to play with, you run the following statements from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or an SQL client such as [psql](connect-query-with-psql-editor.md) to create a table with sample data:
...
- Read the [important notices about logical replication in Neon](guides-logical-replication-neon.md#important-notices) before you begin
...
You can verify that logical replication is enabled by running the following query from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or an SQL client such as [psql](connect-query-with-psql-editor.md):
...
It's recommended that you create a dedicated Postgres role for replicating data. The role must have the `REPLICATION` privilege. The default Postgres role created with your Neon project and roles created using the Neon CLI, Console, or API are granted membership in the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role, which has the required `REPLICATION` privilege.
...
The following Neon API method creates a role. To view the API documentation for this method, refer to the [Neon API reference](reference-cli-roles.md).
...
4. Select an SSL mode. You will most frequently choose `require` or `verify-ca`. Both of these options always require encryption. The `verify-ca` mode requires a certificate. Refer to [Connect securely](connect-connect-securely.md) for information about the location of certificate files you can use with Neon.
5. Under **Advanced**:
...
You can use the following script in a new [Snowflake worksheet](https://docs.snowflake.com/en/user-guide/ui-worksheet) to create the entities. This script is provided as part of [Airbyte's Snowflake connector setup guide](https://docs.airbyte.com/integrations/destinations/snowflake#setup-guide).
   ...
   **Note**: If you want, you can edit the script to change the password to a more secure password and to change the names of other resources. If you do rename entities, make sure to follow [Sbowflake identifier requirements](https://docs.snowflake.com/en/sql-reference/identifiers-syntax).
