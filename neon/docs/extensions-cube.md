...
> The document details the cube extension for Neon, explaining its functionality for multidimensional data storage and querying within the Neon database environment.

## Source

- [The cube extension HTML](https://neon.com/docs/extensions/cube): The original HTML version of this documentation
...
For efficient querying of `cube` data, especially on large tables, [GiST indexes](https://neon.com/postgresql/postgresql-indexes/postgresql-index-types#gist-indexes) are highly recommended. They can make queries faster when using operators like `&&`, `@>`, `<@`, and the distance operators.
