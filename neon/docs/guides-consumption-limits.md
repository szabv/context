...
When setting up your integration's billing solution with Neon, you may want to impose some hard limits on how much storage or compute resources a given project can consume. For example, you may want to cap how much usage your free plan users can consume versus pro or enterprise users. With the Neon API, you can use the `quota` key to set usage limits for a variety of consumption metrics. These limits act as thresholds after which all active computes for a project are [suspended](guides-consumption-limits.md#suspending-active-computes).

## Metrics and quotas

By default, Neon tracks a variety of consumption metrics at the project level. If you want to set quotas (max limits) for these metrics, you need to explicitly [configure](guides-consumption-limits.md#configuring-quotas) them.
...
To find the current usage level for any of these metrics, see [querying metrics](guides-consumption-limits.md#querying-metrics-and-quotas).
...
- `active_time_seconds` &#8212; Sets the maximum amount of time your project's computes are allowed to be active during the current billing period. It excludes time when computes are in an idle state due to [scale to zero](reference-glossary.md#scale-to-zero).
...
In addition to the configurable limits that you can set, Neon also sets certain branch size limits by default. You might notice these limits in a [Get Project](guides-consumption-limits.md#retrieving-details-about-a-project) response:
...
- [Set quotas when you create the project](guides-consumption-limits.md#set-quotas-when-you-create-the-project)
- [Update an existing project](guides-consumption-limits.md#update-an-existing-project)
...
- `autoscaling_limit_max_cu` &#8212; Sets a hard limit on how much compute an endpoint can consume in response to increased demand. For more info on min and max cpu limits, see [Autoscaling](guides-autoscaling-guide.md).
- `suspend_timeout_seconds` &#8212; Sets how long an endpoint's allotted compute will remain active with no current demand. After the timeout period, the endpoint is suspended until demand picks up. For more info, see [Scale to Zero](guides-scale-to-zero-guide.md).
