...
> The document "Connect MCP Clients to Neon" outlines the steps for configuring and establishing a connection between MCP clients and the Neon database, detailing necessary settings and authentication procedures specific to Neon's environment.
...
- [Claude Desktop](ai-connect-mcp-clients-to-neon.md#claude-desktop)
- [Claude Code](ai-connect-mcp-clients-to-neon.md#claude-code)
- [Cursor](ai-connect-mcp-clients-to-neon.md#cursor)
- [Windsurf (Codeium)](ai-connect-mcp-clients-to-neon.md#windsurf-codeium)
- [Cline (VS Code extension)](ai-connect-mcp-clients-to-neon.md#cline-vs-code-extension)
- [Zed](ai-connect-mcp-clients-to-neon.md#zed)
- [VS Code (with GitHub Copilot)](ai-connect-mcp-clients-to-neon.md#vs-code-with-github-copilot)
- [ChatGPT](ai-connect-mcp-clients-to-neon.md#chatgpt)
...
For Local MCP Server setup, you also need a Neon API key. See [Neon API Keys documentation](manage-api-keys.md#creating-api-keys).
...
> By default, the Remote MCP Server connects to your personal Neon account. To connect to an organization's account, you must authenticate with an API key. For more information, see [API key-based authentication](ai-neon-mcp-server.md#api-key-based-authentication).
...
> Replace `<YOUR_NEON_API_KEY>` with your actual Neon API key which you obtained from the [prerequisites](ai-connect-mcp-clients-to-neon.md#prerequisites) section
...
1.  Open Cursor. Create a `.cursor` directory in your project root if needed.
2.  Create or open the `mcp.json` file in the `.cursor` directory.
3.  Add the "Neon" server entry within the `mcpServers` object. Replace `<YOUR_NEON_API_KEY>` with your actual Neon API key which you obtained from the [prerequisites](ai-connect-mcp-clients-to-neon.md#prerequisites) section:
    ```json
    {
      "mcpServers": {
        "Neon": {
...
> For [streamable HTTP responses](ai-connect-mcp-clients-to-neon.md#streamable-http-support) instead of SSE, you can specify the `https://mcp.neon.tech/mcp` endpoint instead of `https://mcp.neon.tech/sse`.
...
5. Configure the Neon Server:
   - Enter **Neon** in the **Name** field.
   - In the **Command** field, enter the following, replacing `<YOUR_NEON_API_KEY>` with your actual Neon API key obtained from the [prerequisites](ai-connect-mcp-clients-to-neon.md#prerequisites) section:
     ```bash
...
For detailed instructions on utilizing the Neon MCP server with GitHub Copilot in VS Code, including a step-by-step example on generating an Azure Function REST API, refer to [How to Use Neon MCP Server with GitHub Copilot in VS Code](https://neon.com/guides/neon-mcp-server-github-copilot-vs-code).
...
- **Remote MCP server:**
  ...
  > By default, the Remote MCP Server connects to your personal Neon account. To connect to an organization's account, you must authenticate with an API key. If your client supports it, provide the key in the `Authorization` header. For more information, see [API key-based authentication](ai-neon-mcp-server.md#api-key-based-authentication).
...
- **Local MCP server:**
  ...
  Add the following JSON configuration within the `mcpServers` section of your client's `MCP` configuration file, replacing `<YOUR_NEON_API_KEY>` with your actual Neon API key obtained from the [prerequisites](ai-connect-mcp-clients-to-neon.md#prerequisites) section:
...
Once connected, you can start interacting with your Neon Postgres databases using natural language commands within your chosen MCP client. Explore the [Supported Actions (Tools)](ai-neon-mcp-server.md#supported-actions-tools) of the Neon MCP Server to understand the available functionalities.

## Resources

- [MCP Protocol](https://modelcontextprotocol.org)
- [Neon API Reference](https://api-docs.neon.tech/reference/getting-started-with-neon-api)
- [Neon API Keys](manage-api-keys.md#creating-api-keys)
