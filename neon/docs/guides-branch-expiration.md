...
Branch expiration allows you to set automatic deletion timestamps on branches. When a branch reaches its expiration time, it is automatically deleted.
   ...
   **Tip** Quick guide: **Console:** When creating a branch, **Automatically delete branch after** is checked by default with 1 day selected. You can choose 1 hour, 1 day, or 7 days, or uncheck to disable. When updating an existing branch, you can select a custom date and time. **CLI:** Use `--expires-at` with [RFC 3339 format](guides-branch-expiration.md#timestamp-format-requirements) (e.g., `2025-07-15T18:02:16Z`). Note: Expiration must be explicitly set; there is no default. **API:** Use `expires_at` with [RFC 3339 format](guides-branch-expiration.md#timestamp-format-requirements) (e.g., `2025-07-15T18:02:16Z`). Note: Expiration must be explicitly set; there is no default.
...
Related docs:
- [Branching with the Neon CLI](guides-branching-neon-cli.md)
- [Branching with the Neon API](manage-branches.md#branching-with-the-neon-api)
- [Manage branches](manage-branches.md)
- [Branching workflows](introduction-branching.md#branching-workflows)
...
- **CLI** - Use the `--expires-at` flag when creating or updating a branch with [RFC 3339](guides-branch-expiration.md#timestamp-format-requirements) format. Note: Expiration must be explicitly set; there is no default.
- **API** - Use the `expires_at` parameter with [RFC 3339](guides-branch-expiration.md#timestamp-format-requirements) format. Note: Expiration must be explicitly set; there is no default.

See the [Examples](guides-branch-expiration.md#examples) section below for detailed usage of each method.
...
- **`expires_at`** (optional) - Timestamp for automatic deletion in [RFC 3339](guides-branch-expiration.md#timestamp-format-requirements) format
...
- **`expires_at`** - The scheduled deletion timestamp ([RFC 3339](guides-branch-expiration.md#timestamp-format-requirements) format)
