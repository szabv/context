...
- You must supply an [API key](manage-api-keys.md), your [project ID](reference-glossary.md#project-id), and the [endpoint ID](reference-glossary.md#endpoint-id)
...
User actions that activate an idle compute include [connecting from a client such as psql](connect-query-with-psql-editor.md), running a query on your database from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md), or accessing the compute via the [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api).
