...
We use the `pg_dump` and `pg_restore` utilities, which are part of the Postgres client toolset. `pg_dump` works by dumping both the schema and data in a custom format that is compressed and suitable for input into `pg_restore` to rebuild the database.
   ...
   **Note**: You can also replicate data from Supabase for a near-zero downtime migration. See [Replicate data from Supabase](guides-logical-replication-supabase-to-neon.md).
...
- A Neon project to move the data to.
  ...
  For detailed information on creating a Neon project, see [Create a project](manage-projects.md#create-a-project). Make sure to create a project with the same Postgres version as your Supabase deployment.
...
To illustrate the migration workflow, we use the [LEGO Database](import-import-sample-data.md#lego-database). This database contains information about LEGO sets, parts, and themes.
...
1. Connect to your Neon project using the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or a Postgres client like [psql](connect-query-with-psql-editor.md).
...
- **CSV export/import**
  ...
  For smaller datasets or specific tables, you might consider exporting to CSV from Supabase and then importing to Neon. See [Import data from CSV](import-import-from-csv.md) for more details on this method.
