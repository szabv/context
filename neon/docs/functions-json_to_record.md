...
> The document explains the usage of the Postgres `json_to_record()` function within Neon, detailing how it converts JSON data into a set of columns, facilitating structured data retrieval from JSON objects.

## Source

- [Postgres json_to_record(https://neon.com/docs/functions/json_to_record): The original HTML version of this documentation
...
- [json_populate_record](functions-json_populate_record.md): This function can also be used to create records using values from a `JSON` object. The difference is that `json_populate_record` requires the record type to be defined beforehand, while `json_to_record` needs the type definition inline.
