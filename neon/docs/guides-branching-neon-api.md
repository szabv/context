...
A Neon API request requires an API key. For information about obtaining an API key, see [Create an API key](manage-api-keys.md#create-an-api-key). In the examples below, `$NEON_API_KEY` is specified in place of an actual API key, which you must provide when making a Neon API request.
...
- The `endpoints` attribute creates a compute, which is required to connect to the branch. Neon supports `read_write` and `read_only` compute types. A branch can be created with or without a compute. You can specify `read_only` to create a [read replica](guides-read-replica-guide.md).
- The `branch` attribute specifies the parent branch.
- The `parent_id` can be obtained by listing the branches for your project. See [List branches](guides-branching-neon-api.md#list-branches-with-the-api). The `parent_id` is the `id` of the branch you are branching from. A branch `id` has a `br-` prefix. You can branch from your Neon project's default branch or a non-default branch.
...
- The `project_id` for a Neon project is found on the **Settings** page in the Neon Console, or you can find it by listing the projects for your Neon account using the Neon API.
- The `branch_id` can be found by listing the branches for your project. The `<branch_id>` is the `id` of a branch. A branch `id` has a `br-` prefix. See [List branches](guides-branching-neon-api.md#list-branches-with-the-api).
...
You can verify that a branch is deleted by listing the branches for your project. See [List branches](guides-branching-neon-api.md#list-branches-with-the-api). The deleted branch should no longer be listed.
...
For more detailed information about branch expiration, including updating and removing expiration timestamps, see [Branch expiration](guides-branch-expiration.md).
