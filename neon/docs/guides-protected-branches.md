...
- New passwords are automatically generated for Postgres roles on branches created from protected branches. [See below](guides-protected-branches.md#new-passwords-generated-for-postgres-roles-on-child-branches).
- With additional configuration steps, you can apply IP Allow restrictions to protected branches only. The [IP Allow](introduction-ip-allow.md) feature is available on the Neon [Scale](introduction-plans.md) plan. See [below](guides-protected-branches.md#how-to-apply-ip-restrictions-to-protected-branches).
- Protected branches are not [archived](guides-branch-archiving.md) due to inactivity.

The protected branches feature is available on Neon [paid plans](introduction-plans.md).
...
Please note that resetting or restoring a child branch from a protected parent branch preserves passwords for matching Postgres roles on the child branch. Please refer to the feature notes below for more.
   ...
   **Important** Feature notes: - The "new password" feature for child branches was released on July, 31, 2024. If you have existing CI scripts that create branches from protected branches, please be aware that passwords for matching Postgres roles on those newly created branches will now differ. If you depend on those passwords being the same, you'll need to make adjustments to get the correct connection details for those branches. - After a branch is created, the up-to-date connection string is returned in the output of the [Create Branch GitHub Action](guides-branching-github-actions.md#create-branch-action). - The [Reset Branch GitHub Action](guides-branching-github-actions.md#reset-from-parent-action) also outputs connection string values, in case you are using this action in your workflows. - The Neon CLI supports a [connection-string](reference-cli-connection-string.md) command for retrieving a branch's connection string. - Prior to September, 6, 2024, resetting or restoring a child branch from a protected parent branch restored passwords for matching Postgres roles on the child branch to those used on the protected parent branch. As of September, 6, 2024, passwords for matching Postgres roles on the child branch are preserved when resetting or restoring a child branch from a protected parent branch.

## How to apply IP restrictions to protected branches

On plans that support it, you can use the protected branches feature in combination with Neon's [IP Allow](introduction-ip-allow.md) feature to apply IP access restrictions to protected branches only. The basic setup steps are:

1. [Define an IP allowlist for your project](guides-protected-branches.md#define-an-ip-allowlist-for-your-project)
2. [Restrict IP access to protected branches only](guides-protected-branches.md#restrict-ip-access-to-protected-branches-only)
3. [Set a branch as protected](guides-protected-branches.md#set-a-branch-as-protected) (if you have not done so already)
...
For details about specifying IP addresses, see [How to specify IP addresses](manage-projects.md#how-to-specify-ip-addresses).
