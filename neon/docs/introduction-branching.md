...
Each Neon project is created with a [root branch](reference-glossary.md#root-branch) called `main`. The first branch that you create is branched from the project's root branch. Subsequent branches can be branched from the root branch or from a previously created branch.
...
The following video demonstrates creating a branch in the Neon Console. For step-by-step instructions, see [Create a branch](manage-branches.md#create-a-branch).



You can integrate branching into your development workflows and toolchains using the Neon CLI, API, or GitHub Actions. If you use Vercel, you can use the [Neon-managed Vercel integration](guides-neon-managed-vercel-integration.md) to create a branch for each preview deployment.
...
Create branches with TTL by [setting an expiration date](guides-branch-expiration.md). Perfect for temporary development and testing environments that need automatic deletion.
...
You can configure your restore window in the Neon Console under **Settings** > **Storage** > **Instant restore**. See [Configure restore window](manage-projects.md#configure-your-restore-window).
...
- [Reset from parent](guides-reset-from-parent.md): Reset a branch to match its parent
- [Time Travel queries](guides-time-travel-assist.md): Run SQL queries against your database's past state
