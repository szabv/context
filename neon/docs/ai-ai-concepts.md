...
> The "AI Concepts" document outlines foundational artificial intelligence principles relevant to Neon, detailing how these concepts integrate with Neon's database functionalities to enhance data processing and management.
...
Neon supports the [pgvector](extensions-pgvector.md) Postgres extension, which enables the storage and retrieval of vector embeddings directly within your Postgres database. When building AI applications, installing this extension eliminates the need to extend your architecture to include a separate vector store. Installing the `pgvector` extension simply requires running the following `CREATE EXTENSION` statement from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or any SQL client connected to your Neon Postgres database.
...
For detailed information about using `pgvector`, refer to our guide: [The pgvector extension](extensions-pgvector.md).
