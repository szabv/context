...
The Neon API is a REST API. It provides resource-oriented URLs, accepts form-encoded request bodies, returns JSON-encoded responses, and supports standard HTTP response codes, authentication, and verbs.
   ...
   **Tip** AI Rules available: Working with AI coding assistants? Check out our [AI rules for the Neon API](ai-ai-rules-neon-api.md) to help your AI assistant understand authentication, rate limiting, and best practices when working with the Neon API.

## Authentication

The Neon API uses API keys to authenticate requests. You can view and manage API keys for your account in the Neon Console. For instructions, refer to [Manage API keys](manage-api-keys.md).
...
You can use the [Neon API reference](https://api-docs.neon.tech/reference/getting-started-with-neon-api) to execute Neon API requests. Select an endpoint, enter an API key token in the **Bearer** field in the **Authorization** section, and supply any required parameters and properties. For information about obtaining API keys, see [Manage API keys](manage-api-keys.md).
...
- [Manage API keys with the Neon API](manage-api-keys.md#manage-api-keys-with-the-neon-api)
- [Manage projects with the Neon API](manage-projects.md#manage-projects-with-the-neon-api)
- [Manage branches with the Neon API](manage-branches.md#branching-with-the-neon-api)
- [Manage computes with the Neon API](manage-computes.md#manage-computes-with-the-neon-api)
- [Manage roles with the Neon API](https://neon.com/docs/manage/users#manage-roles-with-the-neon-api)
- [Manage databases with the Neon API](manage-databases.md#manage-databases-with-the-neon-api)
- [View operations with the Neon API](manage-operations.md#operations-and-the-neon-api)

   **Important**: When using the Neon API programmatically, you can poll the operation `status` to ensure that an operation is finished before proceeding with the next API request. For more information, see [Poll operation status](manage-operations.md#poll-operation-status).
