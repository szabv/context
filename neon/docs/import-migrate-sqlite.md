...
- A Neon account and a project. If you don't have one, see [Sign up](get-started-signing-up.md).
- A database created in your Neon project. For instructions, see [Create a database](manage-databases.md#create-a-database).
...
```
   ...
   **Note**: Passing the `endpoint ID` with your password is a required workaround for some Postgres drivers, including the one used by `pgloader`. For more information, see [Connect with an endpoint ID](connect-connection-errors.md#d-specify-the-endpoint-id-in-the-password-field).
...
Connect to your Neon database using [`psql`](connect-query-with-psql-editor.md) or [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) and check the next value for the `books` table's sequence:
