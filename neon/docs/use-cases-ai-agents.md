...
- [Compute scales up and down in real time based on workload](introduction-autoscaling.md)
- [Scale-to-zero ensures that idle databases cost you nothing](introduction-scale-to-zero.md) while remaining instantly accessible
...
Every app needs authentication, and agents shouldn't have to reinvent it. **[Neon Auth](neon-auth-overview.md) lets you build secure, multi-tenant systems [without extra glue code](https://neon.com/blog/databutton-neon-integration)**.

It issues JWTs that your agent or front-end can use directly in database queries or through the [Neon Data API](data-api-get-started.md). Each token maps to a Postgres role, enforcing granular access at the data level. And because Neon Auth supports standard JWKS configuration, you can also plug in external providers.



## A PostgREST-Compatible Data API, Built In

Giving your agents direct access to the database is simple with the [Neon Data API](data-api-get-started.md). It exposes each database (and every branch) as a REST endpoint you can query over HTTPS. Fully PostgREST-compatible.
...
**Vibe coders experiment constantly, going back and forward between versions - and sometimes breaking things. Neon's [branching](introduction-branching.md) and [snapshots API](ai-ai-database-versioning.md) turn this into a feature, not a risk.**

Branching, built on our copy-on-write storage, enables [instant point-in-time recovery](introduction-branch-restore.md) for any database. Developers and agents can migrate schemas or revert mistakes without complex restores.
...
For instructions on using the Neon API to provision and manage backends on behalf of your users, see [Neon for Platforms Documentation](guides-platform-integration-intro.md). Don't hesitate to [contact us](https://neon.com/contact-sales) as well.

To learn more about the Agent Plan, [see the details on this page](https://neon.com/programs/agents#agent-plan-pricing) or [fill out the application form directly, at the top of this page](use-cases-ai-agents.md#agent-form).
