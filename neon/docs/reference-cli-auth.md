...
Before running the `auth` command, ensure that you have [installed the Neon CLI](reference-cli-install.md).
...
An alternative to authenticating using `neon auth` is to provide an API key when running a CLI command. You can do this using the global `--api-key` option or by setting the `NEON_API_KEY` variable. See [Global options](reference-neon-cli.md#global-options) for instructions.
