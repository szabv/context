...
- [List view](extensions-pg-extensions.md)
- [Install an extension](extensions-pg-extensions.md#install-an-extension)
- [Update an extension](extensions-pg-extensions.md#update-an-extension-version)

**Need an extension we don't have?** 📩 [Request an extension](extensions-pg-extensions.md#request-an-extension)

## AI / Machine Learning

- [pg_tiktoken](extensions-pg_tiktoken.md): Tokenize data in Postgres using the OpenAI tiktoken library
- [pgrag](extensions-pgrag.md): Create end-to-end Retrieval-Augmented Generation (RAG) pipelines
- [pgvector](extensions-pgvector.md): Store vector embeddings and perform vector similarity search in Postgres

## Analytics

- [pg_mooncake](extensions-pg_mooncake.md): Adds columnstore tables and DuckDB execution for fast analytics in Postgres.
...
- [timescaledb](extensions-timescaledb.md): Enables Postgres as a time-series database for efficient storage and retrieval of time-series data
...
- [pgrowlocks](extensions-pgrowlocks.md): Provides a function that shows row locking information for a specified table, useful in concurrency and deadlock debugging
...
- [anon](extensions-postgresql-anonymizer.md): Provides data masking and anonymization capabilities for protecting personally identifiable information (PII) and sensitive data in Postgres databases
- [citext](extensions-citext.md): Provides a case-insensitive character string type that internally calls lower when comparing values in Postgres
- [cube](extensions-cube.md): Implements the cube data type for representing multidimensional cubes in Postgres
- [earthdistance](extensions-earthdistance.md): Provides cube-based and point-based approaches to calculating great circle distances on the surface of the Earth
- [hstore](extensions-hstore.md): Implements an hstore data type for storing and manipulating sets of key-value pairs within a single Postgres value
- [intagg](https://www.postgresql.org/docs/16/intagg.html): Provides an integer aggregator and enumerator for Postgres
- [intarray](extensions-intarray.md): Offers functions and operators for manipulating and searching arrays of integers within Postgres
...
- [ltree](extensions-ltree.md): Provides data types for representing labels of data stored in a hierarchical tree-like structure and facilities for searching through label trees
- [pg_graphql](extensions-pg_graphql.md): Adds GraphQL support to Postgres, allowing you to query your database via GraphQL
...
- [pg_uuidv7](extensions-pg_uuidv7.md): Enables creating valid UUID Version 7 values in Postgres, enabling globally unique identifiers with temporal ordering
...
- [tablefunc](extensions-tablefunc.md): Contains functions that return tables (multiple rows), including crosstab, which can pivot row data into columns dynamically
- [unaccent](extensions-unaccent.md): A text search dictionary that removes accents from characters, simplifying text search in Postgres
...
- [uuid-ossp](extensions-uuid-ossp.md): Provides functions to generate universally unique identifiers (UUIDs) in Postgres, supporting various UUID standards
- [wal2json](extensions-wal2json.md): A Postgres logical decoding plugin that converts Write-Ahead Log (WAL) changes into JSON objects
- [xml2](extensions-xml2.md): Enables XPath queries and XSLT functionality directly within Postgres, enabling XML data processing
...
- [pgrowlocks](extensions-pgrowlocks.md): Provides a function that shows row locking information for a specified table, which can aid in concurrency and deadlock debugging
...
- [cube](extensions-cube.md): Implements a data type for representing multidimensional cubes in Postgres
- [earthdistance](extensions-earthdistance.md): Provides cube-based and point-based approaches to calculating great circle distances on the surface of the Earth
- [h3](extensions-postgis-related-extensions.md#h3-and-h3-postgis): Integrates Uber's H3 geospatial indexing system that combines the benefits of a hexagonal grid with S2's hierarchical subdivisions
- [h3_postgis](extensions-postgis-related-extensions.md#h3-and-h3-postgis): A PostGIS extension for H3, enabling advanced spatial analysis and indexing
- [pgrouting](extensions-postgis-related-extensions.md#pgrouting): Extends PostGIS/Postgres databases, providing geospatial routing and other network analysis functionality
- [postgis](extensions-postgis.md): Extends Postgres to allow GIS (Geographic Information Systems) objects to be stored in the database, enabling spatial queries directly in SQL
...
- [postgis_sfcgal](extensions-postgis-related-extensions.md#postgis-sfcgal): Provides support for advanced 3D geometries in PostGIS, based on the SFCGAL library
- [postgis_tiger_geocoder](extensions-postgis-related-extensions.md#postgis-tiger-geocoder): Enables geocoding and reverse geocoding capabilities in PostGIS using TIGER/Line data
...
- [btree_gin](extensions-btree_gin.md): Provides GIN operator classes that implement B-tree equivalent behavior
- [btree_gist](extensions-btree_gist.md): Provides GiST index operator classes that implement B-tree equivalent behavior
...
- [pg_prewarm](extensions-pg_prewarm.md): Allows manual preloading of relation data into the Postgres buffer cache, reducing access times for frequently queried tables
- [pg_repack](extensions-pg_repack.md): Lets you remove bloat from tables and indexes, and optionally restore the physical order of clustered indexes
...
- [neon](extensions-neon.md): Provides functions and views designed to gather Neon-specific metrics
- [pg_stat_statements](extensions-pg_stat_statements.md): Tracks planning and execution statistics for all SQL statements executed, aiding in performance analysis and tuning
- [pgstattuple](extensions-pgstattuple.md): Offers functions to show tuple-level statistics for tables, helping identify bloat and efficiency opportunities
...
- [online_advisor](extensions-online_advisor.md): Recommends indexes, extended statistics, and prepared statements based on your actual query workload
...
- [citext](extensions-citext.md): Provides a case-insensitive character string type that internally calls lower when comparing values in Postgres
- [dict_int](extensions-dict_int.md): Provides a text search dictionary template for indexing integer data in Postgres
- [fuzzystrmatch](extensions-fuzzystrmatch.md): Provides several functions to determine similarities and distance between strings in Postgres
- [pg_search](extensions-pg_search.md): An Elasticsearch alternative for full-text search and analytics on Postgres
- [pg_trgm](extensions-pg_trgm.md): Provides functions and operators for determining the similarity of alphanumeric text based on trigram matching, and index operator classes for fast string similarity search
...
- [pgcrypto](extensions-pgcrypto.md): Offers cryptographic functions, allowing for encryption and hashing of data within Postgres
...
- [neon_utils](extensions-neon-utils.md): Provides a function for monitoring how Neon's Autoscaling feature allocates vCPU in response to workload
- [pg_cron](extensions-pg_cron.md): Lets you schedule and manage periodic jobs directly in your Postgres database
- [pgtap](https://pgtap.org/documentation.html): A unit testing framework for Postgres, enabling sophisticated testing of database queries and functions
- [refint](https://www.postgresql.org/docs/current/contrib-spi.html): Provides functions for maintaining foreign key constraints

## Remote data access

- [dblink](extensions-dblink.md): Provides functions for accessing and manipulating data in remote databases from within Postgres
- [postgres_fdw](extensions-postgres_fdw.md): Access data in remote Postgres databases as local tables
