...
Every agent project maps to one Neon project with a designated [root branch](reference-glossary.md#root-branch) that serves as the production database.
...
- Maintains a consistent database connection string through Neon's restore mechanism — see [How restore works](ai-ai-database-versioning.md#how-restore-works) for details
...
> See the [poll operation status](manage-operations.md#poll-operation-status) documentation for related information.
...
- [Project](reference-glossary.md#project): Neon project that owns branches, computes, and snapshots, and more
- [Branch](reference-glossary.md#branch): An isolated database environment with its own data and schema that you can connect to and modify
- [Snapshot](reference-glossary.md#snapshot): An immutable, point-in-time backup of a branch's schema and data. Read-only until restored
- [Root branch](reference-glossary.md#root-branch): A branch with no parent (typically named `main` or `production`). The only type of branch from which snapshots can be created
- [Operations](manage-operations.md#operations-and-the-neon-api): Backend operations that return operation IDs you must poll to completion
