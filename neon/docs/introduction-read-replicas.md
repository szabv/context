...
You can instantly create read replicas for any branch in your Neon project and configure the amount of vCPU and memory allocated to each. Read replicas also support Neon's [Autoscaling](introduction-autoscaling.md) and [Scale to Zero](introduction-scale-to-zero.md) features, providing you with the same control over compute resources that you have with your primary compute.
...
- **They are cost-efficient**: With no additional storage or transfer of data, costs associated with storage and data transfer are avoided. Neon's read replicas also benefit from Neon's [Autoscaling](introduction-autoscaling.md) and [Scale to Zero](manage-computes.md#scale-to-zero-configuration) features, which allow you to manage compute usage.
...
You can create read replicas using the Neon Console, [Neon CLI](reference-neon-cli.md), or [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api), providing the flexibility required to integrate read replicas into your workflow or CI/CD processes.
...
For more details and how to connect to a read replica, see [Create and manage Read Replicas](guides-read-replica-guide.md).

## Read Replica architecture

The following diagram shows how your primary compute and read replicas send read requests to the same Pageserver, which is the component of the [Neon architecture](introduction-architecture-overview.md) that is responsible for serving read requests.
