...
You can use the [sample weather dataset from TimescaleDB](https://assets.timescale.com/docs/downloads/weather_small.tar.gz) and load it into your Neon database using [psql](connect-query-with-psql-editor.md).
...
```
   ...
   **Note**: Please be aware that Neon's [Scale to Zero](guides-scale-to-zero-guide.md) feature may affect the running of scheduled jobs. It may be necessary to start the compute before running a job.
