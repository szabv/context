...
For a fully managed solution, see the dedicated [Grafana Cloud integration guide](guides-grafana-cloud.md).
...
2. **Enter your Endpoint URL** based on your platform:
   ...
   **For Grafana Cloud**:
   ...
   - See the [Grafana Cloud guide](guides-grafana-cloud.md) for details
   ...
   **For New Relic**:
   ...
   - See [New Relic's endpoint documentation](https://docs.newrelic.com/docs/opentelemetry/best-practices/opentelemetry-otlp/#configure-endpoint-port-protocol) for other regions
      ...
      **Note**: When you configure an OTLP endpoint URL in Neon, you should provide only the **base URL** of your collector or observability platform. The OpenTelemetry Collector automatically appends the correct signal-specific paths:
...
3. **Create dashboards**: You can visualize using [Grafana Drilldown apps](https://grafana.com/docs/grafana/latest/explore/simplified-exploration/) or use the dashboard JSON from [Grafana Cloud Documentation](guides-grafana-cloud.md#create-a-monitoring-dashboard)
...
- **Metrics**: Look for the auto-created `compute-host-metrics` and `sql-metrics` services
   ...
   **Note**: Neon computes only send logs and metrics when they are active. If the [Scale to Zero](introduction-scale-to-zero.md) feature is enabled and a compute is suspended due to inactivity, no logs or metrics will be sent during the suspension. This may result in gaps in your data. If you notice missing data, check if your compute is suspended. You can verify a compute's status as `Idle` or `Active` on the **Branches** page in the Neon console, and review **Suspend compute** events on the **System operations** tab of the **Monitoring** page. Additionally, if you are setting up the OpenTelemetry integration for a project with an inactive compute, you'll need to activate the compute before it can send data. To activate it, simply run a query from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or any connected client.
...
For a complete list of all metrics and log fields exported by Neon, see the [Metrics and logs reference](reference-metrics-logs.md).
