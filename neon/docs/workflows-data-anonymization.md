...
> The "Data Anonymization" document outlines the process and techniques for anonymizing sensitive data within Neon databases, ensuring compliance with privacy regulations while maintaining data utility for analysis.

## Source

- [Data anonymization HTML](https://neon.com/docs/workflows/data-anonymization): The original HTML version of this documentation
...
- **PostgreSQL Anonymizer integration**: Uses the [PostgreSQL Anonymizer extension's](extensions-postgresql-anonymizer.md) masking functions
- **Branch-specific rules**: You can define different masking rules for each anonymized Neon branch

   **Info** Static versus dynamic masking: This feature uses **static masking**, which permanently transforms data in the branch when anonymization runs. Unlike dynamic masking (which masks data during queries), static masking creates an actual masked copy of the data. To get fresh data from the parent, create a new anonymized branch.
...
After creation, the Console loads the [Data Masking](workflows-data-anonymization.md#manage-masking-rules) page where you define and execute anonymization rules for your branch.
...
4. When you are ready, click `Apply masking rules` to start the anonymization job. You can monitor its progress on this page or via the [API](workflows-data-anonymization.md#get-anonymization-status).
...
For complete API documentation with request/response examples, see the [API reference](workflows-data-anonymization.md#api-reference) section below.
...
4. [Connect](connect-connect-from-any-app.md) your development environment to the anonymized branch.
...
- [Neon branching overview](introduction-branching.md)
...
## Automate data anonymization with GitHub Actions
   ...
   **Important**: The GitHub Actions workflow below uses manual SQL commands with PostgreSQL Anonymizer. For automation using the new Console/API approach documented above, wait for upcoming post-beta improvements to better support automated anonymization.
...
- The following GitHub repository secrets:
   ...
   **Tip**: The Neon GitHub integration can configure these secrets automatically. See [Neon GitHub integration](guides-neon-github-integration.md).
...
Create a file at `.github/workflows/create-anon-branch.yml` (or similar) with the following content. It implements the same masking rules we used in the manual approach:
   ...
   **Note**: This simple workflow example covers the basics. For production use, consider enhancing it with error handling, retry logic, and additional security controls.
