...
> The document outlines the developer experience with Neon, detailing the setup process, integration capabilities, and tools available for efficient database management and development within the Neon environment.

## Source

- [Developer experience with Neon HTML](https://neon.com/docs/get-started/dev-experience): The original HTML version of this documentation
...
Neon's branching feature lets you branch your data like you branch code. Neon branches are full database copies, including both schema and data — but we also support [schema-only branches](guides-branching-schema-only.md#schema-only-branching-example) for those working with sensitive data. You can instantly create database branches for integration with your development workflows.



You can build your database branching workflows using the [Neon CLI](reference-neon-cli.md), [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api), or [GitHub Actions](guides-branching-github-actions.md). For example, this example shows how to create a development branch from `production` with a simple CLI command:
...
These are just a few of the extensions supported by Neon. Explore all supported extensions [here](extensions-extensions-intro.md).

Extensions can be installed with a simple `CREATE EXTENSION` command from Neon's [SQL Editor](get-started-query-with-neon-sql-editor.md) or any SQL client; for example:
...
- **GitHub Actions**
  ...
  Neon provides the GitHub Actions for working with database branches, which you can add to your CI workflows. To learn more, see [Automate branching with GitHub Actions](guides-branching-github-actions.md).
