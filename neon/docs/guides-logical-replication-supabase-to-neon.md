...
1. **Obtain Neon NAT Gateway IP Addresses**: See [NAT Gateway IP addresses](introduction-regions.md#nat-gateway-ip-addresses) for the IP addresses for your Neon project's region. You will need to allow connections from these IP addresses in your Supabase project.
...
Publications are a fundamental part of logical replication in Postgres. They define what will be replicated. You can run the following SQL statements in your Supabase SQL Editor or using [psql](connect-query-with-psql-editor.md) to create a publication for the tables you want to replicate.
...
1. Use the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md), `psql`, or another SQL client to connect to your Neon database.
