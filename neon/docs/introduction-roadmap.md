...
- **Console support for data anonymization**: Neon supports the Postgres `anon` extension for [data anonymization](workflows-data-anonymization.md). We're bringing that support to the console.

Other features you would like to see? [Let us know](introduction-roadmap.md#share-your-thoughts).
...
- AWS and Azure region expansion — let us know where you want to see Neon next: [Request a region](introduction-regions.md)
...
- **Snapshot scheduling**: Automate snapshots with daily, weekly, or monthly schedules with configurable retention periods. Available on paid plans (excluding the Agent plan). [Learn more](guides-backup-restore.md).
...
- **AI Agent Plan**: An AI agent pricing plan for platforms that need to provision thousands of databases. [Learn more](use-cases-ai-agents.md).
...
- **Branch expiration management**: Set a time-to-live (TTL) for Neon branches to simplify branch cleanup and management, see our [branch expiration guide](guides-branch-expiration.md).
...
- **Data API**: Neon's Data API feature, powered by PostgREST, is open to all Neon users. [Learn more](data-api-get-started.md).
- **Monitoring platform support**: Neon supports exporting metrics and Postgres logs to any OpenTelemetry-compatible backend, like New Relic. For details, refer to our [OpenTelemetry docs](guides-opentelemetry.md).
- **Claimable Databases & Neon Luanchpad**: A new way for SaaS vendors to partner with Neon to offer instant Postgres databases. Let your users create Postgres databases — no registration required. [Learn more about Neon Launchpad](reference-neon-launchpad.md), and see our [Claimable database integration guide](workflows-claimable-database-integration.md).
...
- **Import Data Assistant**: The [Import Data Assistant](import-import-data-assistant.md) makes data import easier and faster.
...
- **Support for [pg_search](extensions-pg_search.md)**: We partnered with [ParadeDB](https://www.paradedb.com/) to bring `pg_search` to Neon, delivering up to 1,000x faster full-text search inside Postgres on version 17. [Read the announcement](https://neon.com/blog/pgsearch-on-neon).
- **MACC-eligibility on Azure**: Neon Postgres purchases made through the Azure Marketplace are now counted toward your Microsoft Azure Consumption Commitment (MACC). [Learn more](introduction-billing-azure-marketplace.md#microsoft-azure-consumption-commitment-macc).
...
- **HIPAA compliance**: You can [enable HIPAA compliance](security-hipaa.md) on any Neon project. Learn more about Neon's compliance milestones on our [Compliance page](security-compliance.md).
...
- **Vercel preview deployment support**: We added support for preview deployments with our **Vercel-Managed Integration**. See the [Vercel-Managed Integration guide](guides-vercel-managed-integration.md).
...
- **Save your connection details to [1Password](https://1password.com/)**: See [Save your connection details to 1Password](connect-connect-from-any-app.md#save-your-connection-details-to-1password).
- **Query monitoring in the console**: Monitor your [active queries](introduction-monitor-active-queries.md) and [query performance](introduction-monitor-query-performance.md) in the Neon Console.
- **Schema-only branches**: Create branches that include only your database schema—ideal for workflows involving sensitive data. This feature is now available in Early Access. [Learn more](guides-branching-schema-only.md).
- Support for the [postgres_fdw](extensions-postgres_fdw.md), [dblink](extensions-dblink.md), and [pg_repack](extensions-pg_repack.md) Postgres extensions.
...
- **Schema Diff API**: Neon now supports schema checks in agentic systems and deployment pipelines with the new schema diff API endpoint. Learn more about [Schema Diff](guides-schema-diff.md), which is also available via the console and CLI.
...
- **Support for [pg_cron](extensions-pg_cron.md)**: Schedule and manage periodic jobs directly in your Postgres database with this extension.
...
- **Higher connection limits for autoscaling configurations**: Postgres `max_connections` are now much higher. [Learn more](connect-connection-pooling.md#connection-limits-without-connection-pooling).
- **PgBouncer `default_pool_size` scaling**: The `default_pool_size` is now set according to your compute's `max_connections` setting. Previously, it was fixed at `64`. [Learn more](connect-connection-pooling.md#neon-pgbouncer-configuration-settings).
...
- **Archive storage**: To minimize storage costs, we now support automatic archiving of inactive branches (snapshots of your data) in cost-efficient object storage. For more about this feature, see [Branch archiving](guides-branch-archiving.md).
- **Organizations GA**: Organization Accounts are now generally available. Create a new organization, transfer over your projects, invite your team and get started collaborating. Refer to our [Organizations docs](manage-organizations.md) to learn more.
- **Private Networking**: Private and secure network access to your compute resources without traversing public networks. Support for AWS PrivateLink is available in [Public Beta](guides-neon-private-networking.md).
- **Schema Diff GitHub Action**: This action leverages our [Schema Diff](guides-schema-diff.md) feature to compare database schemas across branches and post the differences as a comment on your pull request, streamlining the review process. It's also supported with our [Neon GitHub integration](guides-neon-github-integration.md).
- **Import Data Assistant**: Helps you migrate data to Neon from other Postgres databases. All you need to get started is a connection string for your existing database. See [Import Data Assistant](import-import-data-assistant.md) for instructions.
...
- **Neon in the Vercel Marketplace**: Neon is now a first-party native integration in the Vercel Marketplace. This integration lets Vercel users add Postgres to their projects and manage billing directly through Vercel. For details, see the [Vercel-Managed Integration guide](guides-vercel-managed-integration.md).
- **Archive storage on the Free plan**: Archive storage is now available on the Free plan for automatically archiving inactive branches. This feature helps minimize storage costs, allowing us to expand the Free plan even further. Learn more in [Branch Archiving](guides-branch-archiving.md).
...
- **ISO27110 & ISO27701 compliance**: These new certifications add to our growing list of compliance achievements. For more about Neon's compliance milestones, see [Compliance](security-compliance.md).
...
- **SQL Editor AI features**: We added AI features to the Neon SQL Editor, including SQL generation, AI-generated query names, and an AI assistant that will fix your queries. [Learn more](get-started-query-with-neon-sql-editor.md#ai-features).
- **Data migration support with inbound logical replication**: We've introduced inbound logical replication as the first step toward enabling seamless, low-downtime migrations from your current database provider to Neon. This feature allows you to use Neon as your development environment, taking advantage of developer-friendly tools like branching and our [GitHub integration](guides-neon-github-integration.md), even if you keep production with your existing provider. To get started, explore our guides for replicating data from AlloyDB, CloudSQL, and RDS. See [Replicate data to Neon](guides-logical-replication-guide.md#replicate-data-to-neon). Inbound logical replication also supports migrating data between Neon projects, useful for version, region, or account migrations. See [Replicate data from one Neon project to another](guides-logical-replication-neon-to-neon.md).

For more of the latest features and fixes, check our [Changelog](changelog.md), published weekly. Or watch for our Changelog email, also sent out weekly. You can also subscribe to updates using our [RSS feed](https://neon.com/docs/changelog/rss.xml).
...
Learn more and sign up on the [Early Access Program page](introduction-early-access.md).
