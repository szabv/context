...
> The document outlines the process for migrating a PostgreSQL database from Digital Ocean to Neon, detailing steps for exporting data, configuring Neon's settings, and importing the database to ensure a seamless transition.

## Source

- [Migrate from Digital Ocean Postgres to Neon HTML](https://neon.com/docs/import/migrate-from-digital-ocean): The original HTML version of this documentation
...
- A Neon project to move the data to.
  For detailed information on creating a Neon project, see [Create a project](manage-projects.md#create-a-project). Make sure to create a project with the same Postgres version as your Digital Ocean deployment.
...
- Review our guide on [Importing data from Postgres](import-migrate-from-postgres.md) for more comprehensive information on using `pg_dump` and `pg_restore`.

## Prepare your Digital Ocean database

This section describes how to prepare your Digital Ocean database for exporting data.

To illustrate the migration workflow, we populate the Digital Ocean database with the [LEGO dataset](import-import-sample-data.md#lego-database). This database contains information about LEGO sets, parts, and themes.
...
1. Connect to your Neon database using the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or `psql`.
...
- **CSV export/import**
  ...
  For smaller datasets or specific tables, you might consider exporting to CSV from Digital Ocean and then importing to Neon. See [Import data from CSV](import-import-from-csv.md) for more details on this method.
