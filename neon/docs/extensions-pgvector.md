...
> The document details the pgvector extension for Neon, enabling users to store and perform similarity searches on vector data within PostgreSQL databases.

## Source

- [The pgvector extension HTML](https://neon.com/docs/extensions/pgvector): The original HTML version of this documentation
...
You can enable the `pgvector` extension by running the following `CREATE EXTENSION` statement in the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or from a client such as [psql](connect-query-with-psql-editor.md) that is connected to Neon.
...
To check the latest supported `pgvector` version on Neon, visit our [Postgres extension page](extensions-extensions-intro.md). You can install one version back from that version.
...
```
   ...
   **Note**: The `pgvector` extension supports some specialized types other than `VECTOR` for storing embeddings. See [HNSW vector types](extensions-pgvector.md#hnsw-vector-types), and [IVFFlat vector types](extensions-pgvector.md#ivfflat-vector-types).
...
- [HNSW](extensions-pgvector.md#hnsw)
- [IVFFLAT](extensions-pgvector.md#ivfflat)
...
- `maintenance_work_mem`
  ...
  In Postgres, the `maintenance_work_mem` setting determines the maximum memory allocation for tasks such as `CREATE INDEX`. The default `maintenance_work_mem` value in Neon is set according to your Neon [compute size](manage-computes.md#how-to-size-your-compute).
...
- Specifying an appropriate number of [probes](extensions-pgvector.md#hnsw-query-options) when querying. A higher number is better for recall, and a lower is better for speed. A good starting point is `sqrt(lists)`.
...
- `maintenance_work_mem`
  ...
  In Postgres, the `maintenance_work_mem` setting determines the maximum memory allocation for tasks such as `CREATE INDEX`. The default `maintenance_work_mem` value in Neon is set according to your Neon [compute size](manage-computes.md#how-to-size-your-compute). For a table that shows the `maintenance_work_mem` setting by compute size, see [Parameter settings that differ by compute size](reference-compatibility.md#parameter-settings-that-differ-by-compute-size).
