...
We recommend setting up one project per user, rather than, for example, using a branch per customer. A Neon [project](manage-overview.md) serves as the logical equivalent of an "instance" but without the management overhead. Here's why we suggest this design:
...
- **Automated backups to your own S3**  
  If you must keep your own data copy, you can [schedule regular backups](manage-backups-aws-s3-backup-part-2.md) using tools like `pg_dump` in conjunction with GitHub Actions.
...
This section assumes you already have an S3 bucket. If you need instructions on how to create a bucket, refer to the [create an S3 bucket](manage-backups-aws-s3-backup-part-1.md) documentation.
