...
Related docs:
- [Archive storage](introduction-architecture-overview.md#archive-storage)
- [Branches list command (Neon CLI)](reference-cli-branches.md#list)
...
- Is a **protected branch** ([learn more](guides-protected-branches.md)).
...
- [Connecting to or querying the branch from a client or application](connect-connect-from-any-app.md)
- [Querying the branch from the Neon SQL Editor](get-started-query-with-neon-sql-editor.md)
- [Viewing the branch on the Tables page in the Neon Console](guides-tables.md)
- [Creating a child branch](manage-branches.md#create-a-branch)
- [Creating a role on a branch](manage-roles.md#create-a-role)
- [Creating a database on a branch](manage-databases.md#create-a-database)
- [Reset the branch from its parent](manage-branches.md#reset-a-branch-from-parent)
- [Performing a restore operation on a branch](https://neon.com/docs/guides/branch-restore)
- [Setting the branch as protected](guides-protected-branches.md)
- Running [Neon CLI](reference-neon-cli.md) commands or [Neon API](https://api-docs.neon.tech/reference/getting-started-with-neon-api) calls that access the branch
...
If you know when a branch should be deleted, set an expiration date rather than wait for automatic archiving. This guarantees automatic deletion at the specified time and works well for CI/CD pipelines and temporary environments. See [Branch expiration](guides-branch-expiration.md) for details.

## Disabling branch archiving

You cannot fully disable branch archiving, but you can prevent a branch from being archived by defining it as a **protected branch**. For instructions, see [Set a branch as protected](manage-branches.md#set-a-branch-as-protected). Protected branches are supported on Neon paid plans.
...
For related information, see [System operations](manage-operations.md).

You can also monitor branch archiving using the Neon CLI or Neon API.

Tab: CLI

The Neon CLI [branches list](reference-cli-branches.md#list) command shows a branch's `Current State`. Branch states include:
