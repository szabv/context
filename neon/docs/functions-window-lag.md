...
> The document explains the usage of the Postgres `lag()` window function within Neon, detailing its syntax and application for accessing data from preceding rows in a result set.

## Source

- [Postgres lag(https://neon.com/docs/functions/window-lag): The original HTML version of this documentation
...
- [lead](functions-window-lead.md) - Access values from subsequent rows in a result set. Similar to `lag()` but looks ahead in the partition instead of behind.
