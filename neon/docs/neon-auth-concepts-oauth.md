...
Neon Auth comes with Google and GitHub OAuth providers pre-configured for authentication. When users sign in with these providers, their accounts are automatically connected, allowing you to access their connected accounts and make API calls on their behalf.
   ...
   **Info**: You cannot connect a user's accounts with shared OAuth keys. You must set up your own OAuth client ID and client secret in the Neon Auth dashboard. For more details, see [Production OAuth setup](neon-auth-best-practices.md#production-oauth-setup).
...
See [Manage OAuth providers via API](neon-auth-api.md#manage-oauth-providers-via-api) for detailed documentation and examples of all available endpoints.
