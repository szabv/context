...
> The "Manage database access" documentation outlines procedures for configuring and controlling user access to databases within the Neon platform, detailing steps for setting permissions and managing roles to ensure secure database operations.

## Source

- [Manage database access HTML](https://neon.com/docs/manage/database-access): The original HTML version of this documentation
...
This Postgres role is automatically assigned the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role, which allows creating databases, roles, and reading and writing data in all tables, views, and sequences. Any user created with the Neon Console, Neon API, or Neon CLI is also assigned the `neon_superuser` role.
...
You can create roles with limited access via SQL. Roles created with SQL are created with the same basic [public schema privileges](manage-database-access.md#public-schema-privileges) granted to newly created roles in a standalone Postgres installation. These users are not assigned the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role. They must be selectively granted permissions for each database object.
...
4. Assign the `readonly` or `readwrite` role to those users to grant them the privileges associated with those roles. For example, assign the `readonly` role to `readonly_user1`, and the `readwrite` role to `readwrite_user1`.
   ...
   **Note**: You can remove a role from a user at any time to revoke privileges. See [Revoke privileges](manage-database-access.md#revoke-privileges).
...
This section describes how to create a read-only role with access to a specific database and schema. An SQL statement summary is provided at the end.
   ...
   **Info**: In Postgres, access must be granted at the database, schema, and object level. For example, to grant access to a table, you must also grant access to the database and schema in which the table resides. If these access permissions are not defined, the role will not be able access the table.

To create a read-only role:

1. Connect to your database from an SQL client such as [psql](connect-query-with-psql-editor.md), [pgAdmin](https://www.pgadmin.org/), or the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md). If you need help connecting, see [Connect from any client](connect-connect-from-any-app.md).
...
2. Create a `readonly` role using the following statement.
   ...
   The password should have at least 12 characters with a mix of lowercase, uppercase, number, and symbol characters. For detailed password guidelines, see [Manage roles with SQL](manage-roles.md#manage-roles-with-sql).

      **Note**: Neon also supports the `NOLOGIN` option: `CREATE ROLE role_name NOLOGIN;` This allows you to define roles that cannot authenticate but can be granted privileges.
...
1. Connect to the database **on the parent branch** from an SQL client such as [psql](connect-query-with-psql-editor.md), [pgAdmin](https://www.pgadmin.org/), or the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md). If you need help connecting, see [Connect from any client](connect-connect-from-any-app.md).
...
3. Grant the `dev_users` role privileges on the database:
   ...
   You now have a `dev_users` role on your parent branch, and the role is not assigned to any users. This role will now be included in all future branches created from this branch.
      ...
      **Note**: The `GRANT` statement above does not grant privileges on existing schemas, tables, sequences, etc., within the database. If you want the `dev_users` role to access specific schemas, tables, etc., you need to grant those permissions explicitly.
...
4. Create a development branch. Name it something like `dev1`. See [Create a branch](manage-branches.md#create-a-branch) for instructions.

5. Connect to the database **on the development branch** with an SQL client. Be mindful that a child branch connection string differs from a parent branch connection string. The branches reside on different hosts. If you need help connecting to your branch, see [Connect from any client](connect-connect-from-any-app.md).
