...
> The Neon Private Networking documentation outlines the setup and configuration of private networking within Neon, enabling secure and isolated network environments for database instances.

## Source

- [Neon Private Networking HTML](https://neon.com/docs/guides/neon-private-networking): The original HTML version of this documentation

**Comingsoon** Private Networking availability: Private Networking is available on Neon's [Scale](introduction-plans.md#scale) plan. If you're on a different plan, you can request a trial from the **Network Security** page in your project's settings.
...
- You must be a Neon [Business](introduction-plans.md#business) and [Scale](introduction-plans.md#scale) account user, and your user account must be [Neon organization](manage-organizations.md) Admin account. You'll encounter an access error if you attempt the setup from a personal Neon account or on a Neon plan that does not offer Private Networking.
- **Ensure that your client application is deployed on AWS in the same region as the Neon database you plan to connect to.** The Private Networking feature is available in all [Neon-supported AWS regions](introduction-regions.md#aws-regions). Both your private access client application and Neon database must be in one of these regions.
...
- Install the Neon CLI. You will use it to add your VPC endpoint ID to your Neon organization. For installation instructions, see [Neon CLI — Install and connect](reference-cli-install.md).
...
## Create an AWS VPC endpoint
       ...
       **Important**: Do not enable **private DNS names** for the VPC endpoint until [Step 3](guides-neon-private-networking.md#enable-private-dns). You must add the VPC endpoint to your Neon organization first, as described in [Step 2](guides-neon-private-networking.md#add-your-vpc-endpoint-id-to-your-neon-organization).
...
In the following example, the VPC endpoint ID is assigned to a Neon organization in the specified AWS region using the [neon vpc endpoint](reference-cli-vpc.md#the-vpc-endpoint-subcommand) command.
...
You can use the [Assign or update a VPC endpoint](https://api-docs.neon.tech/reference/assignorganizationvpcendpoint) API to assign a VPC endpoint ID to a Neon organization. You will need to provide your Neon organization ID, region ID, VPC endpoint ID, and a [Neon API key](manage-api-keys.md).
    ...
    Optionally, you can limit access to a Neon project by allowing connections only from a specific VPC endpoint. For instructions, see [Assigning a VPC endpoint restrictions](guides-neon-private-networking.md#assigning-a-vpc-endpoint-restriction).
...
To block access via the Neon CLI, use the [neon projects update](reference-cli-projects.md#update) command with the `--block-public-connections` option.
...
After adding a restriction, you can check the status of the VPC endpoint to view the restricted project using the [vpc endpoint status command](reference-cli-vpc.md#the-vpc-endpoint-subcommand). You will need to provide your VPC endpoint ID, region ID, and Neon organization ID.
...
The Neon API supports managing project restrictions using the [Assign or update a VPC endpoint restriction](https://api-docs.neon.tech/reference/assignprojectvpcendpoint) endpoint. You will need to provide your VPC endpoint ID, Neon project ID, and a [Neon API key](manage-api-keys.md).
...
For more details and examples, see [Neon CLI commands — vpc](reference-cli-vpc.md).
...
The Private Networking feature supports a maximum of **10 private networking configurations per AWS region**. Supported AWS regions are listed [above](guides-neon-private-networking.md#create-an-aws-vpc-endpoint).
