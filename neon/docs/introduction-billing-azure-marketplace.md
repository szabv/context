...
- [Deploying Neon on Azure](azure-azure-deploy.md)

To get started, see [Deploying Neon on Azure](azure-azure-deploy.md).

## Neon pricing plans and overages

Neon pricing plans include allowances for compute, storage, and projects. For details on each plan's allowances, see [Neon plans](introduction-legacy-plans.md). If you exceed these allowances on a paid plan, overage charges will apply to your monthly bill. You can track your usage on the **Billing** page in the Neon Console. For guidance, see [Monitoring Billing](introduction-monitor-usage.md).

   **Note**: Currently, only Neon [legacy plans](introduction-legacy-plans.md) are supported on Azure. Neon's latest [pricing plans](introduction-plans.md) will be introduced on Azure at a later date.

## Enterprise plan support on Azure

Neon's **Enterprise Plan** is designed for large teams with unique requirements that aren't covered by Neon's self-serve plans. For details, see the [Enterprise Plan](introduction-plans.md#enterprise). To explore this option, contact our [Sales](https://neon.com/contact-sales) team to discuss a custom private offer available through the Azure Marketplace.
...
To stop billing for Neon on Azure, you can remove your Neon resource. For instructions, see [Deleting a Neon resource in Azure](azure-azure-manage.md#deleting-a-neon-resource-in-azure).
