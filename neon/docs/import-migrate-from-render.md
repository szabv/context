...
> The document outlines the steps required to migrate a PostgreSQL database from Render to Neon, detailing the necessary configurations and commands to ensure a smooth transition within Neon's infrastructure.

## Source

- [Migrate from Render to Neon Postgres HTML](https://neon.com/docs/import/migrate-from-render): The original HTML version of this documentation
...
- A Neon project to move the data to.
  ...
  For detailed information on creating a Neon project, see [Create a project](manage-projects.md#create-a-project). Make sure to create a project with the same Postgres version as your Render deployment.
...
- Review our guide on [Migrating data from Postgres](import-migrate-from-postgres.md) for more comprehensive information on using `pg_dump` and `pg_restore`.

## Prepare your Render database

This section describes how to prepare your Render database for exporting data.

To illustrate the migration workflow, we use the [LEGO Database](import-import-sample-data.md#lego-database). This database contains information about LEGO sets, parts, and themes. We load the LEGO database into Render using the [psql](connect-query-with-psql-editor.md) command-line tool.
...
1. Connect to your Neon project using the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or a Postgres client like `psql`.
...
For more information, see [Create a database](manage-databases.md#create-a-database).
...
1. Connect to your Neon database using the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or [psql](connect-query-with-psql-editor.md).
...
- **Logical replication**
  ...
  For larger databases or scenarios where you need to minimize downtime, you might consider using logical replication. See our guide on [Logical replication](guides-logical-replication-guide.md) for more information.

- **CSV export/import**

  For smaller datasets or specific tables, you might consider exporting to CSV from Render and then importing to Neon. See [Import data from CSV](import-import-from-csv.md) for more details on this method.
...
- [Migrating data to Neon](import-migrate-from-postgres.md)
