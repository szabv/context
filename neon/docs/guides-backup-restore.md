...
**Note** Snapshots in Beta: The **Snapshots** feature is now in Beta and available to all users. Snapshot limits: 1 on the Free plan and 10 on paid plans. Automated snapshot schedules are available on paid plans except for the Agent plan. If you need higher limits, please reach out to [Neon support](introduction-support.md).
...
> Instant restore is only supported for root branches. Typically, this is your project's `production` branch. [Learn more](manage-branches.md#root-branch).

Tab: Console

You can restore from any time that falls within your project's [restore window](manage-projects.md#configure-your-restore-window).
...
3. **Restore**
   ...
   For information about removing backup branches, see [Deleting backup branches](introduction-branch-restore.md#deleting-backup-branches).
