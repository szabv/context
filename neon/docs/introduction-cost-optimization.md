...
- **Right-size your compute** — Start by determining the appropriate compute size for your workload. Your compute should be large enough to cache your frequently accessed data (your working set) in memory. A compute that's too small can lead to poor query performance, while an oversized compute wastes resources. See [How to size your compute](manage-computes.md#how-to-size-your-compute) for guidance.

- **Use autoscaling effectively** — Configure [autoscaling](introduction-autoscaling.md) to dynamically adjust compute resources based on demand. Set your minimum size to handle your baseline workload and your maximum to accommodate peak traffic. You only pay for what you use. See [Enable autoscaling](guides-autoscaling-guide.md) for configuration steps.

- **Enable scale to zero** — For non-production environments or databases with intermittent usage, enable [scale to zero](introduction-scale-to-zero.md) to suspend your compute after 5 minutes of inactivity. This can dramatically reduce compute costs for development, testing, and preview environments. See [Configuring scale to zero](guides-scale-to-zero-guide.md).
...
- **Manage child branch storage** — Child branches are billed for the minimum of accumulated data changes or your logical data size—capped at your actual data size. While this prevents charges from exceeding your data size, managing branches effectively still helps minimize costs:
  ...
  - For production workloads, use a [root branch](manage-branches.md#root-branch) instead—root branches are billed on your actual data size.

- **Implement branch lifecycle management** — Review your branches regularly and delete any that are no longer needed. Keeping your branch count under control reduces both storage costs and potential [extra branch charges](introduction-plans.md#extra-branches).
...
- **Use branch expiration** — Set automatic deletion timestamps on temporary branches using [branch expiration](guides-branch-expiration.md) to ensure they're cleaned up when no longer needed.

- **Automate cleanup** — Consider implementing automated cleanup scripts using the [Neon API](manage-branches.md#branching-with-the-neon-api) or [Neon CLI](guides-branching-neon-cli.md) to stay within your plan's branch allowance.
...
- **Monitor your data transfer** — Be aware of how much data you're transferring out of Neon. This includes:
  ...
  - [Logical replication](reference-glossary.md#logical-replication) to any destination, including other Neon databases

- **Review your bill** — If you see unexpectedly high public data transfer charges, [contact support](introduction-support.md) for assistance. Neon does not currently expose detailed data transfer metrics in the Console.
