...
> The "Schema diff tutorial" document guides Neon users through the process of comparing and identifying differences between database schemas using Neon's schema diff tool.

## Source

- [Schema diff tutorial HTML](https://neon.com/docs/guides/schema-diff-tutorial): The original HTML version of this documentation
...
- A Neon account. Sign up [here](get-started-signing-up.md).
- To interact with your Neon database from the command line:
  - Install the [Neon CLI](reference-cli-install.md)
...
1. Create the database.
   ...
   ```
      ...
      **Note**: If you have multiple projects, include `--project-id`. Or set the project context so you don't have to specify project id in every command. Example:
...
You can also launch Schema Diff from the **Restore** page, usually as part of verifying schemas before you restore a branch to its own or another branch's history. See [Instant restore](https://neon.com/docs/guides/branch-restore) for more info.
