...
- [The `neon_stat_file_cache` view](extensions-neon.md#the-neon_stat_file_cache-view)
- [Views for Neon internal use](extensions-neon.md#views-for-neon-internal-use)
...
Neon computes have a Local File Cache (LFC), which is a layer of caching that stores frequently accessed data in the local memory of the Neon compute. Like Postgres [shared buffers](reference-glossary.md#shared-buffers), the LFC reduces latency and improves query performance by minimizing the need to fetch data from Neon storage. The LFC acts as an add-on or extension of Postgres shared buffers. In Neon computes, the `shared_buffers` parameter [scales with compute size](reference-compatibility.md#parameter-settings-that-differ-by-compute-size). The LFC extends the cache memory to approximately 75% of your compute's RAM. To view the LFC size for each Neon compute size, see [How to size your compute](manage-computes.md#how-to-size-your-compute).
...
You can monitor Local File Cache (LFC) usage by installing the `neon` extension on your database and querying the [neon_stat_file_cache](https://neon.com/docs/) view or [using EXPLAIN ANALYZE](extensions-neon.md#view-lfc-metrics-with-explain-analyze). Additionally, you can monitor the [Local file cache hit rate](introduction-monitoring-page.md#local-file-cache-hit-rate) graph on the **Monitoring** page in the Neon console.
