...
> The "Time Travel" documentation for Neon details how users can access historical data states by utilizing the time travel feature, enabling precise data retrieval from specific past points.
...
To change your restore window, see [Configure restore window](manage-projects.md#configure-restore-window).
...
1. Select the branch you want to query against, then select a timestamp, the same as you would to [Restore a branch](guides-time-travel-assist.md#restore-a-branch-to-an-earlier-state).
...
Read more about getting connection strings from the CLI in [Neon CLI commands — connection-string](reference-cli-connection-string.md), and more about setting contexts in [CLI - set-context](reference-cli-set-context.md).
...
- Ephemeral endpoints are created with a .50 CU compute size, which has 0.50 vCPU size with 2 GB of RAM. This is Neon's second smallest compute size. For more about compute sizes in Neon, see [How to size your compute](manage-computes.md#how-to-size-your-compute).
