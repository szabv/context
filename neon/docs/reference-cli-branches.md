...
> The document details Neon CLI commands for managing branches, including creating, listing, and deleting branches within the Neon database environment.

## Source

- [Neon CLI commands — branches HTML](https://neon.com/docs/reference/cli-branches): The original HTML version of this documentation

## Before you begin

- Before running the `branches` command, ensure that you have [installed the Neon CLI](reference-cli-install.md).
...
The `branches` command allows you to list, create, rename, delete, and retrieve information about branches in your Neon project. It also permits setting a branch as the default branch, adding a compute to a branch, adding a [read replica](introduction-read-replicas.md), or perforning a [schema diff](guides-schema-diff.md) between different branches.
...
| --------------------------------- | -------------------------------------------- |
| [list](reference-cli-branches.md#list)                     | List branches                                |
| [create](reference-cli-branches.md#create)                 | Create a branch                              |
| [reset](reference-cli-branches.md#reset)                   | Reset data to parent                         |
| [restore](reference-cli-branches.md#restore)               | Restore a branch to a selected point in time |
| [rename](reference-cli-branches.md#rename)                 | Rename a branch                              |
| [schema-diff](reference-cli-branches.md#schema-diff)       | Compare schemas                              |
| [set-default](reference-cli-branches.md#set-default)       | Set a default branch                         |
| [set-expiration](reference-cli-branches.md#set-expiration) | Set expiration date for a branch             |
| [add-compute](reference-cli-branches.md#add-compute)       | Add replica to a branch                      |
| [delete](reference-cli-branches.md#delete)                 | Delete a branch                              |
| [get](reference-cli-branches.md#get)                       | Get a branch                                 |
...
| `--type`            | Type of compute to add. Choices are `read_write` (the default) or `read_only`. A read-only compute is a [read replica](introduction-read-replicas.md).                                                                                                             | string  |                                                     |
...
| `--expires-at`      | Set an expiration timestamp (RFC 3339 format) for automatic branch deletion. The branch and its compute endpoints are permanently deleted at the specified time.                                                                                                      | string  |                                                     |
   ...
   **Note**: When creating a branch from a protected parent branch, role passwords on the child branch are changed. For more information about this Protected Branches feature, see [New passwords generated for Postgres roles on child branches](guides-protected-branches.md#new-passwords-generated-for-postgres-roles-on-child-branches).
...
- Create a branch with a [read replica](introduction-read-replicas.md) compute.
...
- [Restoring a branch to an earlier point in its history](reference-cli-branches.md#restoring-a-branch-to-an-earlier-point-in-its-own-history-with-backup)
- [Restoring to another branch's head](reference-cli-branches.md#restoring-a-branch-target-to-the-head-of-another-branch-source)
- [Restoring a branch to its parent](reference-cli-branches.md#restoring-a-branch-to-its-parent-at-an-earlier-point-in-time)
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `rename` subcommand supports these options:
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `schema-diff` subcommand supports these options:
...
| `--database`, `--db` | Name of the database for which the schema comparison is performed                             | string |                                                                           |
   ...
   **Note**: The `--no-color` or `--color false` [global option](reference-neon-cli.md#global-options) can be used to decolorize the CLI command output when using CLI commands in CI/CD pipelines.

#### Examples

Examples of different kinds of schema diff operations you can do:

- [Compare to another branch's head](reference-cli-branches.md#compare-to-another-branchs-head)
- [Compare to an earlier point in a branch's history](reference-cli-branches.md#comparing-a-branch-to-an-earlier-point-in-its-history)
- [Compare a branch to its parent](reference-cli-branches.md#comparing-a-branch-to-its-parent)
- [Compare to an earlier point in another branch's history](reference-cli-branches.md#comparing-a-branch-to-an-earlier-point-in-another-branchs-history)
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `set-default` subcommand supports this option:
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `set-expiration` subcommand supports these options:
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `add-compute` subcommand supports these options:

| Option           | Description                                                                                                                                                                                                                                         | Type   |                      Required                       |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | :-------------------------------------------------: |
| `--context-file` | [Context file](reference-cli-set-context.md#using-a-named-context-file) path and file name                                                                                                                                                       | string |                                                     |
| `--project-id`   | Project ID                                                                                                                                                                                                                                          | string | Only if your Neon account has more than one project |
| `--type`         | Type of compute to add. Choices are `read_only` (the default) or `read_write`. A read-only compute is a [read replica](introduction-read-replicas.md). A branch can have a single primary read-write compute and multiple read replica computes. | string |                                                     |
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `delete` subcommand supports this option:
...
In addition to the Neon CLI [global options](reference-neon-cli.md#global-options), the `get` subcommand supports this option:
