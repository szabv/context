...
- [RAM](introduction-monitoring-page.md#ram)
- [CPU](introduction-monitoring-page.md#cpu)
- [Connections count](introduction-monitoring-page.md#connections-count)
- [Database size](introduction-monitoring-page.md#database-size)
- [Deadlocks](introduction-monitoring-page.md#deadlocks)
- [Rows](introduction-monitoring-page.md#rows)
- [Replication delay bytes](introduction-monitoring-page.md#replication-delay-bytes)
- [Replication delay seconds](introduction-monitoring-page.md#replication-delay-seconds)
...
CPU is allocated according to the size of your compute or your [autoscaling](guides-autoscaling-guide.md) configuration, if applicable. For example, if your compute size is .25 CU (.25 vCPU with 1 GB RAM), your allocated CPU is always 0.25. With autoscaling, allocated CPU increases and decreases as your compute size scales up and down in response to load. If [scale to zero](guides-scale-to-zero-guide.md) is enabled and your compute transitions to an idle state after a period of inactivity, allocated CPU drops to 0.

**Used**: The amount of CPU used, in [Compute Units (CU)](reference-glossary.md#compute-unit-cu).

If the plotted line regularly reaches the maximum amount of allocated CPU, consider increasing your compute size. To see the compute sizes available with Neon, see [Compute size and autoscaling configuration](manage-computes.md#compute-size-and-autoscaling-configuration).
...
- Implementing [connection pooling](connect-connection-pooling.md), which supports up to 10,000 simultaneous connections
- Optimizing your application's connection management

The connection limit (defined by the Postgres `max_connections` setting) is set according to your Neon compute size configuration. For the connection limit for each Neon compute size, see [How to size your compute](manage-computes.md#how-to-size-your-compute).
...
- **Local file cache size**: This is the size of the LFC, which is determined by the size of your compute. Larger computes have larger caches. For cache sizes, see [How to size your compute](manage-computes.md#how-to-size-your-compute).
