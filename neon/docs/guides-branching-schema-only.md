...
If you have more than one project, you'll need to specify the `--project-id` option. See [Neon CLI - branch create](reference-cli-branches.md#create).
...
1. Start by creating an `employees` table on your Neon project's `main` branch and adding some dummy data. You can do this from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or any SQL client by copying and pasting the following statements:
...
3. Create a schema-only branch following the instructions above. See [Creating schema-only branches](guides-branching-schema-only.md#creating-schema-only-branches). In this example, we've named the branch `employees_schema_only`.
...
Schema-only branches are independent [root branches](reference-glossary.md#root-branch), just like the `production` branch in your Neon project. When you create a schema-only branch, you're creating a new **root branch**.
...
- **Reset from parent is not supported**: With no parent branch, [reset from parent](manage-branches.md#reset-a-branch-from-parent) operations are not supported.
...
- **Branch protection is supported**: Like any other branch, you can enable [branch protection](guides-protected-branches.md) for schema-only branches.

## Schema-only branch allowances

There are certain allowances associated with schema-only branches:

- A schema-only branch is a [root branch](reference-glossary.md#root-branch), and only a certain number of root branches are permitted per Neon project, depending on your Neon plan.
- The `main` root branch created with each Neon project counts toward the _root branch allowance per project_, as do certain [backup branches](reference-glossary.md#backup-branch) created by restore operations.
