...
Neon offers HIPAA compliance as a self-serve feature available to customers on the [Scale](introduction-plans.md) plan.
...
For Neon project creation steps, see [Create a project](manage-projects.md#create-a-project).
...
To create a new HIPAA-compliant Neon project via the [Neon CLI](reference-neon-cli.md), use the `--hipaa` option with the `neon projects create` command, as shown below.
...
If you have trouble enabling HIPAA, contact `hipaa@neon.tech`.
   ...
   **Note**: For information about disabling HIPAA compliance, see [Disabling HIPAA](security-hipaa.md#disabling-hipaa).
...
1. [Neon Console and API audit logs](security-hipaa.md#neon-console-and-api-audit-logs): Captures user actions in the Neon Console and via the Neon API.
2. [Postgres audit logs](security-hipaa.md#postgres-audit-logs-pgaudit): Logged using the [pgAudit](https://www.pgaudit.org/) extension (`pgaudit`) for Postgres.
...
The `pgaudit` extension is preloaded on HIPAA-enabled Neon projects. For extension version information, see [Supported Postgres extensions](extensions-pg-extensions.md).
...
- [Neon Auth](neon-auth-overview.md) – Uses an authentication provider that is not covered under Neon's HIPAA compliance.
- [Data API](data-api-get-started.md) – Hosted outside Neon's HIPAA-compliant infrastructure.
...
_This guide provides a high-level overview of Neon's HIPAA compliance efforts. For more details, please refer to your Business Associate Agreement (BAA) or contact us directly via our [support channels](introduction-support.md)._
