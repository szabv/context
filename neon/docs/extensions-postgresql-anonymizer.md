...
The `anon` extension ([PostgreSQL Anonymizer](https://postgresql-anonymizer.readthedocs.io)) provides data masking and anonymization capabilities to protect sensitive data in Postgres databases. It helps protect personally identifiable information (PII) and other sensitive data, facilitating compliance with regulations such as [GDPR](https://gdpr-info.eu/).
   ...
   **Tip** Looking for a practical guide?: For complete step-by-step workflows on anonymizing data in Neon branches, including manual procedures and GitHub Actions automation, see [data anonymization](workflows-data-anonymization.md).

## Enable the extension

   **Note**: This extension is currently [experimental](extensions-pg-extensions.md#experimental-extensions) and may change in future releases.
...
1. Connect to your Neon database using either the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or an SQL client like [psql](connect-query-with-psql-editor.md)
...
3. Install the extension:
   ...
   **Tip**: When using the Neon Console or API to create branches, the extension is enabled automatically. See the [data anonymization workflow guide](workflows-data-anonymization.md) for details.
...
For complete implementation examples showing how to apply these masking functions in real workflows, see the [data anonymization guide](workflows-data-anonymization.md), which covers:
...
- [Data anonymization workflow guide](workflows-data-anonymization.md) - Practical guide for anonymizing data in Neon branches
