...
3. Create a backup of your Neon database by running the following `pg_dump` command with your Neon database connection string.
   ...
   For more command options, see [Advanced pg_dump and pg_restore options](manage-backup-pg-dump.md#advanced-pgdump-and-pgrestore-options).
...
4. Restore your data to the target database in Neon with `pg_restore`.
   ...
   - `-d`: Specifies the Neon database to connect to. The value is a Neon database connection string. See [Before you begin](manage-backup-pg-dump.md#before-you-begin).
