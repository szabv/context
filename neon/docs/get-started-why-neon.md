...
Neon's [architecture](introduction-architecture-overview.md) separates compute from storage, which enables serverless features like instant provisioning, [autoscaling](get-started-production-readiness.md#autoscaling), [scale to zero](get-started-production-readiness.md#scale-to-zero), and more.
...
[Read more about the benefits of Neon's serverless architecture](introduction-serverless.md) and how it supports database-per-user architectures, variable workloads, database branching workflows, and [AI agents](use-cases-ai-agents.md).
...
Building on Neon is building on Postgres. If you are already running Postgres, getting started is easy. [Import your data](https://neon.com/docs/import/import-intro) and [connect](connect-connect-intro.md). Migrating from other databases like MySQL or MongoDB is just as easy.

If you need to move data, you won't have to tear apart your application to remove proprietary application layers. Neon is pro-ecosystem and pro-integration. We encourage you to build with the frameworks, platforms, and services that best fit your requirements. Neon works to enable that. Check out our ever-expanding collection of [framework](get-started-frameworks.md), [language](get-started-languages.md), and [integration](guides-integrations.md) guides.
...
- **Individual developers** looking for a fast and easy way to set up a Postgres database without the hassle of installation or configuration. Neon's Free plan makes it easy to get started. [Free plan](introduction-plans.md) users get access to all regions and features like connection pooling and branching. When you are ready to scale, you can easily upgrade your account to a paid plan for more computing power, storage, and advanced features.
...
- **AI agents** that need to rapidly provision Postgres databases, execute SQL queries, and efficiently manage Neon infrastructure. With one-second provision times, scale-to-zero compute, and agent-friendly client interfaces, Neon enables AI agents to manage database fleets at scale while keeping costs low. AI agents are on track to surpass humans in the number of databases created on the Neon platform. [Learn more about this use case](use-cases-ai-agents.md).
...
If you are not quite ready to hook up an application, you can explore Neon from the console. Create the `playing_with_neon` table using the Neon [SQL Editor](get-started-query-with-neon-sql-editor.md), run some queries, or create a database branch.

Initially, you'll be signed up for Neon's [Free plan](introduction-plans.md), but you can easily upgrade to one of our paid plans when you're ready.
