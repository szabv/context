...
- A Neon project to move the data to.
  ...
  For detailed information on creating a Neon project, see [Create a project](manage-projects.md#create-a-project). Make sure to create a project with the same Postgres version as your Azure PostgreSQL deployment.
...
3. Click on `Add a Firewall Rule`, which generates a modal to add the range of IP addresses from which we want to allow connections. You will need to perform this step for each of the NAT gateway IP addresses associated with your Neon project's region. For each IP address, create a new rule and fill both the `Start IP` and `End IP` fields with the IP address.
   ...
   Neon uses 3 to 6 IP addresses per region for this outbound communication, corresponding to each availability zone in the region. See [NAT Gateway IP addresses](introduction-regions.md#nat-gateway-ip-addresses) for Neon's NAT gateway IP addresses.
...
You can find the connection details for your database by clicking the **Connect** button on your **Project Dashboard**. See [Connect from any application](connect-connect-from-any-app.md).

### Create the Neon database

To keep parity with the Azure PostgreSQL deployment, create a new database with the same name. See [Create a database](manage-databases.md#create-a-database) for more information.
...
1. Use the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md), [psql](connect-query-with-psql-editor.md), or another SQL client to connect to your Neon database.
...
This ensures a much shorter downtime for the application, as you only need to wait for the last few transactions to be replicated before switching the application over to the Neon database.
   ...
   **Note**: Remember to update any Azure-specific configurations or extensions in your application code to be compatible with Neon. For Neon Postgres parameter settings, see [Postgres parameter settings](reference-compatibility.md#postgres-parameter-settings). For Postgres extensions supported by Neon, see [Supported Postgres extensions](extensions-pg-extensions.md).
...
- **pg_dump and pg_restore**
  ...
  If your database size is not large, you can use the `pg_dump` utility to create a dump file of your database, and then use `pg_restore` to restore the dump file to Neon. Please refer to the [Migrate from Postgres](import-migrate-from-postgres.md) guide for more information on this method.
...
- [Neon logical replication guide](guides-logical-replication-guide.md)
