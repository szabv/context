...
Your Postgres role and roles created in the Neon Console, API, and CLI are granted membership in the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role. Roles created with SQL from clients like [psql](connect-query-with-psql-editor.md), [pgAdmin](https://www.pgadmin.org/), or the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) are only granted the basic [public schema privileges](manage-database-access.md#public-schema-privileges) granted to newly created roles in a standalone Postgres installation. These users must be selectively granted permissions for each database object. For more information, see [Manage database access](manage-database-access.md).
...
- [Neon Console](manage-roles.md#manage-roles-in-the-neon-console)
- [Neon CLI](manage-roles.md#manage-roles-with-the-neon-cli)
- [Neon API](manage-roles.md#manage-roles-with-the-neon-api)
- [SQL](manage-roles.md#manage-roles-with-sql)
...
You can think of roles with `neon_superuser` privileges as administrator roles. If you require roles with limited privileges, such as a read-only role, you can create those roles from an SQL client. For more information, see [Manage database access](manage-database-access.md).
...
This section describes how to create, view, and delete roles in the Neon Console. All roles created in the Neon Console are granted membership in the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role.
...
7. Click **Create**. The role is created and you are provided with the password for the role.
   ...
   **Note**: Role names cannot exceed 63 characters, and some names are not permitted. See [Reserved role names](manage-roles.md#reserved-role-names).
...
6. On the **Reset password** modal, click **Reset**. A reset password modal is displayed with your new password.
   ...
   **Note**: Resetting a password in the Neon Console resets the password to a generated value. To set your own password value, you can reset the password using the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md) or an SQL client like [psql](connect-query-with-psql-editor.md) with the following syntax:

```sql
ALTER USER user_name WITH PASSWORD 'new_password';
```

For password requirements, see [Manage roles with SQL](manage-roles.md#manage-roles-with-sql).

## Manage roles with the Neon CLI

The Neon CLI supports creating and deleting roles. For instructions, see [Neon CLI commands — roles](reference-cli-roles.md). Roles created with the Neon CLI are granted membership in the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role.
...
Roles created with the Neon API are granted membership in the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role.
...
```
   ...
   **Note**: Role names cannot exceed 63 characters, and some role names are not permitted. See [Reserved role names](manage-roles.md#reserved-role-names).
...
Roles created with SQL have the same basic `public` schema privileges as newly created roles in a standalone Postgres installation. These roles are not granted membership in the [neon_superuser](manage-roles.md#the-neonsuperuser-role) role like roles created with the Neon Console, CLI, or API. You must grant these roles the privileges you want them to have.

To create a role with SQL, issue a `CREATE ROLE` statement from a client such as [psql](connect-query-with-psql-editor.md), [pgAdmin](https://www.pgadmin.org/), or the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md).
...
For role creation and access management examples, refer to the [Manage database access](manage-database-access.md) guide.
...
- The Neon CLI [`neon roles create`](reference-cli-roles.md#create) command supports a `--no-login` option.
