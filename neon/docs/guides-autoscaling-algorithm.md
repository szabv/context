...
> The document explains Neon's autoscaling algorithm, detailing how it dynamically adjusts resources based on workload demands to optimize performance and efficiency within the Neon database environment.

## Source

- [Understanding Neon's autoscaling algorithm HTML](https://neon.com/docs/guides/autoscaling-algorithm): The original HTML version of this documentation
...
The key concept behind autoscaling is that compute resizing happens _automatically_ — once you set up your minimum and maximum [compute sizes](https://neon.com/docs/manage/endpoints#how-to-size-your-compute), there's no action required on your part other than [monitoring](introduction-monitoring-page.md) your usage metrics to see if adjustments are needed.
...
Neon's autoscaling algorithm uses two components, the [vm-monitor](reference-glossary.md#vm-monitor) and the [autoscaler-agent](reference-glossary.md#autoscaler-agent), to continuously monitor three key metrics: your average CPU load, your memory usage, and the activity of your [Local File Cache (LFC)](reference-glossary.md#local-file-cache). These metrics determine how your compute resources — the virtual machine that powers your database — should be scaled to maintain performance and efficiency.
