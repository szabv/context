...
- [Postgres jsonb_extract_path(https://neon.com/docs/functions/jsonb_extract_path): The original HTML version of this documentation
...
- [json_extract_path](functions-json_extract_path.md) - The `jsonb_extract_path` function works with the `JSONB` data type, which offers a binary representation of `JSON` data, whereas `json_extract_path` takes a `JSON` value as an input and returns `JSON` too. The `JSONB` variant is typically more performant at query time, which is even more pronounced with larger `JSON` data payloads and frequent path extractions.
