...
- [Developing with Neon on Azure](azure-azure-develop.md)

This guide steps you through deploying Neon as an Azure native service.

   **Note**: You can also create Neon projects in Azure regions without the native service integration—to learn more, see [Create Neon projects in Azure regions without the integration](azure-azure-deploy.md#create-neon-projects-in-azure-regions-without-the-integration).
...
2. Subscribe to the service. You will be directed to the [Create a Neon Serverless Postgres Resource](azure-azure-deploy.md#create-a-neon-resource) page.
...
1. On the **Create a Neon Serverless Postgres Resource** page, enter values for the properties described below.
   ...
   | **Region**                 | Select a region to deploy your Azure resource. This is the region for your Azure resource, not for your Neon projects and data. You will select from [Azure-supported regions](introduction-regions.md#azure-regions) when you create the Neon project in a later step. For example, you can create a Neon resource in the (US) West US 3 region and create a Neon project (Europe) Germany West Central. |
   | **Neon Organization name** | Provide a name for your [Neon Organization](reference-glossary.md#organization), such as a team name or company name. The name you specify will be your [Organization](reference-glossary.md#organization) name in the Neon Console. Your Neon projects will reside in this named organization.                                                                                                        |
...
1. On the **Project** page, enter a name for your Neon project, select a Postgres version, specify a name for your database, and choose a region. We recommend selecting the region closest to your application.
         ...
         **Note**: A Neon organization created via the Azure portal supports creating Neon projects in [Azure regions](introduction-regions.md#azure-regions) only. Neon's AWS regions are not supported with Neon on Azure.
...
1. From here, follow the [Neon Getting Started](get-started-signing-up.md) guide to begin working with your Neon project and get familiar with the platform.
...
To create a Neon project from the console, follow the [Create project](manage-projects.md#create-a-project) steps. Select **Azure** as the **Cloud Service Provider**, and choose one of the available [Azure regions](introduction-regions.md).

Tab: CLI

To create a Neon project using the Neon CLI, use the [neon projects create](reference-cli-projects.md#create) command:

```bash
neon projects create --name mynewproject --region-id azure-eastus2
```

For Azure `region-id` values, see [Regions](introduction-regions.md).
...
For Azure `region_id` values, see [Regions](introduction-regions.md).
