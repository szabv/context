...
- A Neon [paid plan](introduction-plans.md) if you intend to install a dataset larger than 0.5 GB.
- Instructions for each dataset require that you create a database. You can do so from a client such as `psql` or from the [Neon SQL Editor](get-started-query-with-neon-sql-editor.md).

   **Note**: You can also load sample data using the Neon CLI. See [Load sample data with the Neon CLI](import-import-sample-data.md#load-sample-data-with-the-neon-cli).
...
| ----------------------------------------------------------- | ------ | -------- | ---------------- | -------------- |
| [Periodic table data](import-import-sample-data.md#periodic-table-data)                 | 1      | 118      | 17 KB            | 7.2 MB         |
| [World Happiness Index](import-import-sample-data.md#world-happiness-index)             | 1      | 156      | 9.4 KB           | 7.2 MB         |
| [Titanic passenger data](import-import-sample-data.md#titanic-passenger-data)           | 1      | 1309     | 220 KB           | 7.5 MB         |
| [Netflix data](import-import-sample-data.md#netflix-data)                               | 1      | 8807     | 3.2 MB           | 11 MB          |
| [Pagila database](import-import-sample-data.md#pagila-database)                         | 33     | 62322    | 3 MB             | 15 MB          |
| [Chinook database](import-import-sample-data.md#chinook-database)                       | 11     | 77929    | 1.8 MB           | 17 MB          |
| [Lego database](import-import-sample-data.md#lego-database)                             | 8      | 633250   | 13 MB            | 42 MB          |
| [Employees database](import-import-sample-data.md#employees-database)                   | 6      | 3919015  | 34 MB            | 333 MB         |
| [Wikipedia vector embeddings](import-import-sample-data.md#wikipedia-vector-embeddings) | 1      | 25000    | 1.7 GB           | 850 MB         |
| [Postgres air](import-import-sample-data.md#postgres-air-database)                      | 10     | 67228600 | 1.2 GB           | 6.7 GB         |
...
8. Navigate to the directory where you extracted the source file, and run the following command:
   ...
   **Note**: If you encounter a memory error related to the `maintenance_work_mem` setting, refer to [Parameter settings that differ by compute size](reference-compatibility.md#parameter-settings-that-differ-by-compute-size) for how to increase this setting.
...
- [Neon CLI — Install and connect](reference-cli-install.md)
- [PostgreSQL Downloads](https://www.postgresql.org/download/) for `psql`

If you have multiple Neon projects or branches, we recommend setting your Neon CLI project and branch context so that you don't have to specify them explicitly when running a Neon CLI command. See [Neon CLI commands — set-context](reference-cli-set-context.md).
...
2. Load the data using one of the following Neon CLI commands ([projects](reference-cli-projects.md), [branches](reference-cli-branches.md), or [connection-string](reference-cli-connection-string.md)):
