...
> The document explains the usage of the `jsonb_extract_path_text()` function in Neon, detailing how it retrieves text from a JSONB object at a specified path.

## Source

- [Postgres jsonb_extract_path_text(https://neon.com/docs/functions/jsonb_extract_path_text): The original HTML version of this documentation
...
- [jsonb_extract_path](functions-jsonb_extract_path.md) - This is a similar function that can extract data from a `JSONB` object at the specified path. The difference is that it returns a `JSONB` object, while `jsonb_extract_path_text` always returns text. The right function to use depends on what you want to use the output data for.
- [json_extract_path_text](functions-json_extract_path_text.md) - This is a similar function that can extract data from a `JSON` object, (instead of `JSONB`) at the specified path.
