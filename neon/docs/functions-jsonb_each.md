...
> The document explains the usage of the Postgres `jsonb_each()` function, detailing how it iterates over each key-value pair in a JSONB object, specifically for Neon users working with JSON data in PostgreSQL.

## Source

- [Postgres jsonb_each(https://neon.com/docs/functions/jsonb_each): The original HTML version of this documentation
...
- [json_each](functions-json_each.md) - It provides the same functionality as `jsonb_each`, but accepts `JSON` input instead of `JSONB`.
